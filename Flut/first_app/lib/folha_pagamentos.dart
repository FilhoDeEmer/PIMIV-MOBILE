...
class _FolhaPagState extends State<FolhaPag> {
  ...
  Future<void> fetchData() async {
    try {
      ...
      for (var holerite in holerites) {
        ...
        if (holerite['cod_funcionario'] ==
            funcionarioInfos?['codigo_funcionario']) {
          listaFolhasFuncionario.add(holerite);
        }
      }
      var maiorFolhaFunc;
      for (var folha in listaFolhasFuncionario) {
        if (maiorFolhaFunc == null ||
            folha['codFolha'] > maiorFolhaFunc['codFolha']) {
  ...
  Widget build(BuildContext context) {
    ...
    double valorDescontado = funcionarioInfos != null
        ? retornaTotalDescontado(funcionarioInfos!)
        : 0.0;
